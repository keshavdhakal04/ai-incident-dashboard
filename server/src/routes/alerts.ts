...
router.post("/:id/suggestion", (req: Request, res: Response) => {
  ...
  alertStore[index] = { ...alertStore[index], suggestion, updatedAt: new Date().toISOString() };
  res.json({ success: true, data: alertStore[index] });
});

router.patch("/:id/status", (req: Request, res: Response) => {
  const { status } = req.body as { status: AlertStatus };
  const validStatuses: AlertStatus[] = ["open", "accepted", "rejected", "overridden"];
  ...
  alertStore[index] = { ...alertStore[index], status, updatedAt: new Date().toISOString() };
  res.json({ success: true, data: alertStore[index] });
});
